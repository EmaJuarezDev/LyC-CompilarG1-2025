...
void generarWri(FILE *fPun) {
    ...
    if(variable) {
        ...
        fprintf(fPun,"lea DX, _T_%s\nmov AH, 09h\nint 21h\n\n", auxNombre);
    }
    else
        fprintf(fPun,"lea DX, %s\nmov AH, 09h\nint 21h\n\n", topePilAsm);    
...
void generarAsm() {
	...
	for (i = 0; i < posicionAct; i++) {
		...
		/*else if(strcmp(variablePil, "READ") == 0 )
            ...
            fprintf(fCA, "JNB ET_%d\n", cantidadSal);
        }
		else if(strcmp(variablePil, "BGT") == 0) {

            cantidadSal++;
            vecSaltos[cantidadSal] = atoi(vectorPolInv[i + 1]);
            fprintf(fCA, "JNBE ET_%d\n", cantidadSal);
        }
